...
class TestRunner:
    ...
    def run_security_tests(
        self, quick_mode: bool = False, generate_report: bool = False
    ) -> bool:
    ...
    def run_unit_tests(self, coverage: bool = False) -> bool:
        ...
        # Test configuration
        cmd.extend(["tests/", "-v", "--tb=short", "-m", "unit", "--strict-markers"])
...
"""
    ...
    def display_summary(self):
        ...
        for test_name, result in self.results.items():
            status = "✅ PASSED" if result["success"] else "❌ FAILED"
            table.add_row(test_name.title(), status, str(result["exit_code"]))
        ...
        if passed_count == total_count:
            ...
            console.print(
                "❌ [red]Review failed tests before production deployment[/red]"
            )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode for security tests"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.security or args.all:
            passed = runner.run_security_tests(
                quick_mode=args.quick, generate_report=bool(args.report)
            )
