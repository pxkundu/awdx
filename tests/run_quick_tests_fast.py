#!/usr/bin/env python3
"""
⚡ AWDX Fast Quick Test Runner
==============================

Ultra-fast parallel test runner for AWDX modules using mocked/optimized tests.

Usage:
    python tests/run_quick_tests_fast.py [--parallel] [--verbose]
...
Examples:
    python tests/run_quick_tests_fast.py
    python tests/run_quick_tests_fast.py --sequential --verbose
"""
...
def run_test_command(cmd: List[str], test_name: str, timeout: int = 60) -> Tuple[str, bool, str, float]:
...
def run_tests_parallel() -> Dict[str, Tuple[bool, str, float]]:
    ...
    console.print("⚡ [bold blue]Running Fast Tests in Parallel[/bold blue]")
    
    # Define fast test commands
    test_commands = [
        {
            "name": "Unit Tests",
        ...
        },
        {
            ...
            "cmd": [sys.executable, "-m", "pytest", "tests/test_task_integration_fast.py", "-m", "integration", "--strict-markers", "--tb=short", "-q"],
            "timeout": 30
        },
        {
            "name": "Security Tests",
            "cmd": [sys.executable, "-m", "pytest", "tests/test_security_scanner_fast.py", "-m", "security", "--strict-markers", "--tb=short", "-q"],
            "timeout": 30
        }
    ]
...
def run_tests_sequential() -> Dict[str, Tuple[bool, str, float]]:
    ...
    console.print("⚡ [bold blue]Running Fast Tests Sequentially[/bold blue]")
...
def display_results(results: Dict[str, Tuple[bool, str, float]], verbose: bool = False):
    ...
    table = Table(title="🧪 AWDX Fast Test Results")
    ...
    if passed_count == total_count:
        console.print("🎉 [bold green]All fast tests passed![/bold green]")
        console.print("🚀 [green]Your codebase is ready for development![/green]")
    else:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="AWDX Fast Quick Test Runner")
    ...
    console.print(f"⚡ [bold blue]AWDX Fast Quick Test Runner[/bold blue]")
    console.print(f"📁 Project root: {project_root}")
    console.print(f"🔧 Mode: {'Parallel' if args.parallel else 'Sequential'}")
    console.print(f"⚡ Using fast (mocked) test scripts for maximum speed")
    console.print()
